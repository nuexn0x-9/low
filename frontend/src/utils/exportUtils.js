...
export function generateFrameSvg(frame, options = {}) {
  ...
  if (options.includeSafeArea && frame.safeArea) {
    ...
    safeAreaSnippet = `
      <!-- Safe Area Guides -->
      ${sa.top ? `<rect x="0" y="0" width="${w}" height="${sa.top}" fill="rgba(37,99,235,0.08)" stroke="#2563eb" stroke-dasharray="4 4" stroke-width="1" />` : ""}
      ${sa.bottom ? `<rect x="0" y="${h - sa.bottom}" width="${w}" height="${sa.bottom}" fill="rgba(37,99,235,0.08)" stroke="#2563eb" stroke-dasharray="4 4" stroke-width="1" />` : ""}
    `;
  }
